# functions/ai-chat__db.py
# ============================================================
# ФУНКЦИЯ: ai-chat__db.py
# Размер: 406 строк
# Путь в проекте: backend/ai-chat/db.py
...
def get_prices_block() -> str:
    """Загружает цены из БД и формирует компактный блок для SYSTEM_PROMPT.
    Синонимы НЕ включаем — они уже есть в build_rules_prompt, дублирование раздувает контекст."""
    try:
        conn = psycopg2.connect(os.environ['DATABASE_URL'])
        cur = conn.cursor()
