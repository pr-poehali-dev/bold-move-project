...
function RenderBlocks({ blocks, maxWidth }: { blocks: PageBlock[]; maxWidth?: string }) {
  ...
  return (
    <div className={`p-4 mx-auto ${maxWClass[maxWidth ?? "md"] ?? "max-w-md"}`}>
      <div className="flex flex-wrap gap-2">
        {blocks.map(block => {
          ...
          if (block.type === "heading") {
            ...
            return (
              <div key={block.id} className={wClass} style={blockStyle}>
                <p className={`${size} text-white ${alignCls(block.align)}`}>{block.text}</p>
              </div>
            );
          }
          if (block.type === "text") {
            return (
              <div key={block.id} className={wClass} style={blockStyle}>
                <p className={`text-white/70 text-sm leading-relaxed whitespace-pre-wrap ${alignCls(block.align)}`}>{block.text}</p>
              </div>
            );
          }
          ...
          if (block.type === "buttons") {
            const justifyCls = block.align === "center" ? "justify-center" : block.align === "right" ? "justify-end" : "justify-start";
            return (
              <div key={block.id} className={wClass} style={blockStyle}>
          ...
          if (block.type === "divider") {
            if (block.style === "dots") return (
              <div key={block.id} className={wClass} style={blockStyle}>
                <div className="flex justify-center gap-2 py-1"><span className="w-1.5 h-1.5 rounded-full bg-white/20" /><span className="w-1.5 h-1.5 rounded-full bg-white/20" /><span className="w-1.5 h-1.5 rounded-full bg-white/20" /></div>
              </div>
            );
            if (block.style === "space") return <div key={block.id} className={wClass} style={{ ...blockStyle, height: 24 }} />;
            return (
              <div key={block.id} className={wClass} style={blockStyle}>
          ...
          if (block.type === "spacer") {
            return <div key={block.id} className={wClass} style={{ height: block.height }} />;
          }
          if (block.type === "card") {
            return (
              <div key={block.id} className={wClass} style={blockStyle}>
                <div className={`flex flex-col gap-1.5 p-3 rounded-2xl bg-white/[0.03] border border-white/[0.06] ${alignCls(block.align)}`}>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {hasBlocks ? (
          <RenderBlocks blocks={c.blocks!} maxWidth={c.pageSettings?.maxWidth ?? "md"} />
        ) : (
          <>
            {c.photo_url && (
