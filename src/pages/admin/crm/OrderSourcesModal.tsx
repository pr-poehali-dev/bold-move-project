...
interface Props {
  ...
  onUpdate: (id: number, patch: Partial<OrderSource>) => Promise<void>;
  onRemove: (id: number) => Promise<void>;
}

export function OrderSourcesModal({ sources, onClose, onCreate, onUpdate, onRemove }: Props) {
  const t = useTheme();
        ...
        <div className="flex items-center justify-between px-5 py-4"
          ...
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "#10b98120" }}>
              <Icon name="Radio" size={14} style={{ color: "#10b981" }} />
            </div>
            <span className="text-sm font-bold" style={{ color: t.text }}>Источники заявок</span>
          </div>
              ...
              {editId === s.id ? (
                ...
                <div className="rounded-xl p-3 flex items-center gap-3" style={{ background: "#ef444412", border: "1px solid #ef444430" }}>
                  <span className="text-xs flex-1" style={{ color: "#fca5a5" }}>
                    Удалить источник «{s.name}»? В прошлых заявках он сохранится.
                  </span>
