    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          ...
          <div key={c.label} className="bg-[#0a0a16] border border-white/[0.05] rounded-2xl p-5">
            ...
            <div className="text-xs text-white/25 mt-0.5">{c.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-[#0a0a16] border border-white/[0.05] rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-bold text-white">P&L (доходы / расходы)</span>
          </div>
          <div className="space-y-2.5">
            {[
              ...
              <div key={r.label} className="flex justify-between items-center text-sm border-b border-white/[0.04] pb-2">
        ...
        <div className="bg-[#0a0a16] border border-white/[0.05] rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-sm font-bold text-white">Структура затрат</span>
          </div>
          {costPie.length > 0 ? (
            <>
              <div className="flex justify-center mb-3">
                <PieChart width={140} height={140}>
                  <Pie data={costPie} cx={65} cy={65} innerRadius={38} outerRadius={62} dataKey="value" strokeWidth={2} stroke="#0a0a16">
              ...
              <div className="space-y-2">
                {costPie.map(c => (
                  <div key={c.name} className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-white/70">{c.value.toLocaleString("ru-RU")} ₽</span>
                  </div>
                ))}
              </div>
            </>
          ) : (
            <div className="flex items-center justify-center h-32 text-white/20 text-sm flex-col gap-2">
        ...
        <div className="bg-[#0a0a16] border border-white/[0.05] rounded-2xl p-5">
          ...
          <div className="flex justify-center mb-3">
            <PieChart width={140} height={140}>
              <Pie data={statusPie} cx={65} cy={65} innerRadius={38} outerRadius={62} dataKey="value" strokeWidth={2} stroke="#0a0a16">
          ...
          <div className="space-y-1.5">
            {statusPie.map(s => (
              <div key={s.name} className="flex justify-between text-xs">
                ...
                <span className="font-semibold text-white/70">{s.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[#0a0a16] border border-white/[0.05] rounded-2xl p-5">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={combined} margin={{ top: 0, right: 0, left: -25, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ background: "#0e0e1c", border: "1px solid #ffffff10", borderRadius: 10, fontSize: 12 }} labelStyle={{ color: "#fff" }} />
              <Legend wrapperStyle={{ fontSize: 11, color: "#ffffff50", paddingTop: 6 }} />
        ...
        <div className="bg-[#0a0a16] border border-white/[0.05] rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-1 h-4 rounded-full bg-emerald-500" />
            <span className="text-sm font-bold text-white">Выручка / Затраты / Прибыль</span>
          </div>
          {marginData.length > 0 ? (
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={marginData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ background: "#0e0e1c", border: "1px solid #ffffff10", borderRadius: 10, fontSize: 12 }} labelStyle={{ color: "#fff" }} formatter={(v: number) => v.toLocaleString("ru-RU") + " ₽"} />
                <Legend wrapperStyle={{ fontSize: 11, color: "#ffffff50", paddingTop: 6 }} />
            ...
            <div className="flex items-center justify-center h-44 text-white/20 text-sm">Нет данных — добавьте суммы договоров и затраты</div>
          )}
        </div>
      </div>
      ...
      <div className="bg-[#0a0a16] border border-white/[0.05] rounded-2xl p-5">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-1 h-4 rounded-full bg-red-500" />
          <span className="text-sm font-bold text-white">Детализация отказов</span>
          <span className="ml-auto text-xs text-white/25 bg-white/[0.04] px-2 py-0.5 rounded-lg">
            {stats.total_cancel} отказников из {stats.total_all} заявок ({cancelRate}%)
          </span>
        </div>
        {stats.cancel_reasons.length === 0 ? (
          <div className="flex items-center justify-center py-6 gap-3 text-white/20">
                ...
                <div key={i} className="flex items-center gap-3 bg-white/[0.02] rounded-xl p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <span className="text-sm text-white/60 truncate">{r.reason}</span>
                      <span className="text-xs font-bold text-white/60 ml-2">{r.count} ({pct}%)</span>
                    </div>
