...
export function FileField({ label, url, onUploaded, accept = "*" }: {
  ...
  const deleteFile = (idx: number) => setUploadedList(prev => prev.filter((_, i) => i !== idx));
    ...
    <div className="py-2" style={{ borderBottom: `1px solid ${t.border2}` }}>
            ...
            <div key={i} className="flex items-center gap-1 group/file">
              ...
              {renamingIdx === i ? (
                ...
                <>
                  ...
                  <button onClick={() => { renameValRef.current = f.name; setRenamingIdx(i); }}
                    className="opacity-0 group-hover/file:opacity-100 transition p-0.5 rounded hover:text-violet-300"
                    style={{ color: t.textMute }} title="Переименовать">
              ...
              <button onClick={() => deleteFile(i)}
                ...
                style={{ color: t.textMute }} title="Удалить">
                <Icon name="X" size={10} />
              </button>
            </div>
          ))}
      ...
      {lightboxIdx !== null && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center"
          ...
          <img src={imageItems[lightboxIdx].url} alt={imageItems[lightboxIdx].name}
...
export function TagsField({ tags, onSave, editMode = false }: {
  ...
  const deleteTag = (label: string) => onSave(current.filter(tg => tg !== label));
