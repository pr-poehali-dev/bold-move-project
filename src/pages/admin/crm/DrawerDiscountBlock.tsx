...
export function DrawerDiscountBlock({ data, customFinRows, onContractSumUpdated }: Props) {
  ...
  const runComplexityAnalysis = async () => {
    ...
    try {
      ...
      // ЭТАП 2 — Семантика
      setAnalysisStep(2);
      const semanticPromptText = (prompts.semantic || `Ты монтажник натяжных потолков с 15-летним опытом. Говоришь конкретно, без общих фраз.\n\nСписок позиций сметы:\n{items}\n\nОцени РЕАЛЬНУЮ сложность монтажа для ЭТОГО набора работ. Пиши только то, что прямо следует из этих позиций.\n\nПравила:\n- Если в списке нет двухуровневых, многоуровневых, радиусов — не упоминай их\n- Если нет светильников — не пиши про освещение\n- Если нет ниш — не пиши про ниши\n- Каждый риск должен быть прямым следствием конкретной позиции из списка\n- Не пиши про «согласование с клиентом», «соблюдение сроков» и другие общеменеджерские риски — это не монтажные сложности\n\nОцени от 1 до 10 (целое число). Дай 2-3 предложения — ТОЛЬКО про реальные монтажные нюансы этого набора позиций.\n\nФормат: "Оценка: X/10. [Пояснение]"`)
