...
function applyPhoneMask(raw: string): string {
  ...
  // Нормализуем: если начинается с 8 или 7 — заменяем, иначе добавляем 7
...
export function InlineField({ label, value, onSave, type = "text", placeholder = "—", hideLabel }: {
  ...
  const commit = () => {
    ...
    if (isPhone && v && !isPhoneValid(v)) {
      setPhoneErr(true);
      return; // не закрываем — ждём исправления
    }
    setEditing(false);
    setPhoneErr(false);
    onSaveRef.current(v);
  ...
  const handlePhoneChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setPhoneErr(false);
    // Авто-коммит если номер полный
    if (isPhoneValid(masked)) setPhoneErr(false);
  };
