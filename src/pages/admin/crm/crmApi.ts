...
export interface Client {
  ...
  /** Сервисная заявка — мелкая доделка/переделка по сданному объекту, а не полноценный монтаж.
   *  Такие заявки показываются в отдельной вкладке «Сервис» и не попадают в «Монтажи». */
  is_service?: boolean;
  /** Заявка на этапе «Выполнено» проверена — качество и оплата сверены.
   *  Только для этапа "done", на остальных этапах не используется. */
  is_verified?: boolean;
  /** Заявка на этапе «Выполнено» подтверждена — второй слайдер рядом с «Проверено»
