...
function TrialBanner() {
  ...
  const trial = isBusiness && user.trial_until ? new Date(user.trial_until) : null;
  const now   = new Date();
  const balance = user?.estimates_balance ?? 0;
  const expired = trial ? trial < now : false;
  const hoursLeft = trial ? Math.max(0, Math.floor((trial.getTime() - now.getTime()) / 3600000)) : 0;
          ...
          <div className="rounded-3xl p-4 sm:p-5"
            style={{
              background: expired
                ...
                : "linear-gradient(90deg, rgba(251,191,36,0.10), rgba(251,191,36,0.02))",
              border: `1.5px solid ${expired ? "rgba(239,68,68,0.28)" : "rgba(251,191,36,0.28)"}`,
            }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0"
                style={{ background: expired ? "rgba(239,68,68,0.18)" : "rgba(251,191,36,0.18)" }}>
                <Icon name={expired ? "AlertCircle" : "Timer"} size={18}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-white/55 leading-snug">
                  {expired
                    ...
                    : <>Осталось <b className="text-[#fbbf24]">{balance} смет</b> и <b className="text-[#fbbf24]">{daysLeftN > 0 ? `${daysLeftN} дн. ${remainder} ч.` : `${hoursLeft} ч.`}</b> до конца пробного периода.</>
                  }
                </div>
              </div>
            </div>
