...
export default function PlanPage() {
  ...
  // Zoom helpers
  const zoomIn  = useCallback(() => handleSettingChange({ zoom: Math.min(4,  Math.round((state.settings.zoom + 0.2) * 10) / 10) }), [state, handleSettingChange]);
  const zoomOut = useCallback(() => handleSettingChange({ zoom: Math.max(0.3, Math.round((state.settings.zoom - 0.2) * 10) / 10) }), [state, handleSettingChange]);
  ...
  return (
    <div className="flex flex-col bg-[#0f1117] overflow-hidden" style={{ height: "100dvh" }}>
      ...
      <div className="h-6 bg-[#0c0d16] border-t border-white/[0.04] flex items-center px-4 gap-5 shrink-0">
        ...
        <span className="text-[10px] text-white/20 font-mono">Диагоналей: {state.diagonals.length}</span>
