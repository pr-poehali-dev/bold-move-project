...
export default function PlanVariantPicker({ variants, loading, activeVariantId, onSelect, onLoad, onDelete, onRename, onClose }: Props) {
  ...
  return (
    <>
      ...
      >
        {/* Шапка */}
        <div className="flex items-center justify-between px-4 py-3 border-b shrink-0 sticky top-0 z-10"
          style={{ borderColor: "rgba(255,255,255,0.07)", background: "#0e0e1c" }}>
                ...
                <div className="flex items-center gap-2 px-3 py-2 border-t" style={{ borderColor: "rgba(255,255,255,0.06)" }}>
                  {renamingId === v.id ? (
                    ...
                    <>
                      ...
                      <button
                        onClick={() => { setRenamingId(v.id); setRenameName(v.name); }}
