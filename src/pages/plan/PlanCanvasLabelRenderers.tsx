...
export function SegmentItemsBadges({
  ...
  const GAP = S * 0.3;
  // Увеличенный отступ от стены чтобы не перекрывать inline-лейблы с размерами
  // Иконки рисуются с противоположной стороны нормали (внутрь), лейблы — наружу
  // Дополнительный отступ гарантирует, что иконки не залезают на текст
  const OFF = S * 1.8; // было 1.2 — увеличено чтобы не наезжать на лейбл размера
