      ...
      <div className="flex-1 px-4 sm:px-8 py-5 max-w-3xl mx-auto w-full">
        ...
        {!showCreate && (
          <div className="flex flex-col gap-3 mb-5">
            ...
            {/* Фильтр по статусу — скролл везде, на ПК мельче */}
            <div className="flex gap-1 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
              {STATUSES.map(s => (
                <button
                  key={s.id}
                  onClick={() => setFilterStatus(s.id)}
                  className="px-2 py-0.5 rounded-full text-[10px] sm:text-[10px] font-semibold transition whitespace-nowrap shrink-0"
                  style={{
