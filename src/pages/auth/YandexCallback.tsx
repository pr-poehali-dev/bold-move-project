// Страница возврата от Яндекс OAuth — обменивает code на сессию через backend/auth
// (action=yandex-callback), затем логинится тем же токеном, что и обычный вход.
...
export default function YandexCallback() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!code) { setError("Яндекс не передал код авторизации"); return; }
      ...
      try {
        ...
        if (!res.ok || data.error) { setError(data.error || "Не удалось войти через Яндекс"); return; }
        if (linkToken) { navigate("/?profile=1&linked=yandex", { replace: true }); return; }
      ...
      <div className="text-center">
        {error ? (
          ...
          <>
            ...
            <p className="text-sm text-white/50">Входим через Яндекс…</p>
          </>
        )}
      </div>
    </div>
