...
import { CLIENT_ROLES } from "@/context/AuthContext";
...
export default function EstimateContactModal({
  show, done, saving, name, phone, addr, user,
          ...
          {done ? (
            <div className="flex flex-col items-center text-center py-4">
              ...
              <div className="text-white font-bold text-base mb-1">
                {isClient ? "Заявка отправлена!" : "Заявка сохранена в CRM!"}
              </div>
              <div className="text-white/40 text-sm">
                {isClient ? "Мы свяжемся с вами в ближайшее время" : "Открываем карточку клиента..."}
              </div>
            </div>
          ) : isClient ? (
            /* ── Клиент: хочет заказать замер ── */
            <>
              <div className="flex flex-col items-center text-center mb-5">
                <div className="w-14 h-14 rounded-2xl bg-orange-500/15 flex items-center justify-center mb-3">
                  <Icon name="CalendarCheck" size={26} className="text-orange-400" />
                </div>
                <div className="text-white font-bold text-base mb-1">Записаться на бесплатный замер</div>
                <div className="text-white/40 text-sm leading-relaxed">
                  Оставьте контакты — мастер приедет в удобное время
                </div>
              </div>
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-xs text-white/40 mb-1 block">Ваш телефон</label>
                  <PhoneInput value={phone} onChange={onChangePhone} showValidation
                    className="w-full rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none"
                    style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(249,115,22,0.25)" }} />
                </div>
                <div>
                  <label className="text-xs text-white/40 mb-1 block">Адрес объекта</label>
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={onSave} disabled={saving || !isPhoneValid(phone)}
                  ...
                  {saving ? "Отправляем..." : "Записаться на замер"}
                </button>
            ...
            /* ── Компания/менеджер: сохранение клиента в CRM ── */
            <>
              <div className="flex flex-col items-center text-center mb-5">
                <div className="w-14 h-14 rounded-2xl bg-violet-500/15 flex items-center justify-center mb-3">
                  <Icon name="UserPlus" size={26} className="text-violet-400" />
                </div>
                <div className="text-white font-bold text-base mb-1">Сохранить клиента в CRM</div>
                <div className="text-white/40 text-sm leading-relaxed">
                  Укажите данные клиента — заявка появится в вашей воронке
                </div>
              </div>
              <div className="space-y-3">
                <div>
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={onSave} disabled={saving || (!!phone && !isPhoneValid(phone))}
                  ...
                  {saving ? "Сохраняем..." : "Сохранить в CRM"}
                </button>
                <button onClick={onClose}
                  className="px-4 py-2.5 rounded-xl text-sm transition"
                  style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.4)" }}>
