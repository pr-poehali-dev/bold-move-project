...
export function WLPipelineDrawer({ company, onClose, onUpdate, onDelete, onOpenPanel, onRunApiTests, onRequestReceipt }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      {/* Backdrop — только десктоп */}
      <div className="hidden sm:block flex-1 bg-black/40" onClick={onClose} />

      {/* Drawer */}
      <div
        className="w-full flex flex-col overflow-hidden"
        style={{ background: "#0e0e1a", borderLeft: "1px solid rgba(255,255,255,0.07)", maxWidth: "min(480px, 100vw)", height: "100dvh" }}
      ...
      >
        ...
        <div className="px-4 pt-3 border-t border-white/[0.06] flex-shrink-0" style={{ paddingBottom: "max(16px, env(safe-area-inset-bottom))" }}>
          {saveError && (
