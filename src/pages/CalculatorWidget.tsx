...
export default function CalculatorWidget() {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/3 p-5 md:p-8">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-orange-500/15 to-rose-500/15 border border-orange-500/25 p-4 md:p-5">
        <div className="flex flex-col gap-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="text-white/50 text-sm mb-1">Примерная стоимость</div>
              <div className="font-montserrat font-black text-3xl md:text-4xl text-white">{total.toLocaleString("ru")} <span className="text-lg md:text-xl text-orange-400">₽</span></div>
              <div className="text-white/40 text-xs mt-1">Точный расчёт — при бесплатном замере</div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-white/40 text-xs mb-1">Скидка 10% онлайн</div>
              <div className="font-montserrat font-black text-xl md:text-2xl text-orange-400">{totalWithDiscount.toLocaleString("ru")} ₽</div>
            </div>
          </div>
          <a href="#contact" className="flex items-center justify-center gap-1 w-full bg-gradient-to-r from-orange-500 to-rose-500 text-white text-sm font-montserrat font-bold px-4 py-3 rounded-xl hover:scale-[1.02] transition-transform">
            Заказать со скидкой <Icon name="ArrowRight" size={14} />
          </a>
        </div>
      </div>
    </div>
