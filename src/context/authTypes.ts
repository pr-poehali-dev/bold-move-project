...
export interface Permissions {
  ...
  // ── Разделы внутри «Настроек компании» ───────────────────────────────────
  // Раньше были жёстко доступны только владельцу компании — теперь каждый
  // раздел включается отдельным тумблером. Владелец/мастер проходят всегда.
  team_view?:         boolean;
  own_agent_view?:    boolean;
  ...
  // ── Уровень 1: Вкладки воронки заказов (leads/working/measures/installs/done) —
  // ЧИСТО видимость самой вкладки в списке, никак не связана с тем, что внутри нее
  // разрешено. Можно открыть вкладку «Замеры», но не дать ни одного статуса/подэтапа
  // внутри — сотрудник увидит вкладку, но пустую (0 заявок). И наоборот: вкладку
  // можно скрыть полностью, даже если статусы внутри разрешены.
  // Пустой массив / отсутствие ключа = ограничений нет (видны все вкладки).
  allowed_tabs?: string[];
  ...
  // ── Уровень 3: Кастомные подэтапы (order_substatuses) — какие подэтапы внутри
  // вкладки видит/меняет сотрудник. Пустой массив / отсутствие ключа = ограничений
  // нет (видны все подэтапы). Больше НЕ зависит от allowed_statuses — сам родительский
  // статус подэтапа включается отдельно, это независимые переключатели.
  allowed_substatuses?: string[];

  // ── Уровень 2: Видимость заявок по ответственному ───────────────────────
  // "all"        — все заявки компании (поведение по умолчанию, как было раньше)
  // "own"        — только заявки, закреплённые за этим сотрудником
  // "own_free"   — свои + ничьи (новые, без ответственного) — можно взять в работу
  // Отсутствие ключа трактуется как "all", чтобы у существующих сотрудников
  // ничего не изменилось после обновления.
  orders_scope?: "all" | "own" | "own_free";
  // Редактировать разрешено только свои заявки (чужие — только просмотр).
  orders_edit_own_only?: boolean;
  // Может вручную назначить/сменить ответственного у заявки (выпадающий список
  // в карточке). Без этого права ответственный проставляется только автоматически —
  // кто первым тронул заявку. По умолчанию выключено (false/отсутствие ключа) —
  // владелец/мастер это право обходят (см. hasPermission).
  orders_reassign?: boolean;

  // ── Уровень 2: Календарь — какие типы событий видит сотрудник ───────────
  // Пустой массив / отсутствие ключа = видны все типы (как было раньше).
  // Значения совпадают с calendar_events.event_type: next_call, last_call, measure, install
  calendar_event_types?: string[];
  // Какие из доступных типов включены В КАЛЕНДАРЕ ПО УМОЛЧАНИЮ (при первом входе).
  // Отличается от calendar_event_types: тот решает, что сотруднику ВООБЩЕ доступно,
  // а этот — что он видит сразу, не открывая фильтр. Дальше сотрудник может сам
  // включить/выключить типы, его выбор сохраняется локально и имеет приоритет.
  // Пустой массив / отсутствие ключа = по умолчанию показываем все доступные типы.
  calendar_default_event_types?: string[];
...
/**
 * Список вкладок воронки заказов (leads/working/measures/installs/done), видимых
 * пользователю. null — ограничений нет (видны все вкладки). Это ЧИСТО видимость
 * самой вкладки в списке — не связана с тем, какие статусы/подэтапы внутри неё
 * разрешены (см. allowedStatusesOf/allowedSubstatusesOf — независимые переключатели).
 */
export function allowedTabsOf(user: AuthUser | null): string[] | null {
  if (!user || user.is_master) return null;
  if (user.role === "company" || user.role === "installer") return null;
  const list = user.permissions?.allowed_tabs;
  return Array.isArray(list) && list.length > 0 ? list : null;
}

/**
 * Список статусов воронки, разрешённых пользователю (какие заявки сотрудник видит
 * и может редактировать). null — ограничений нет. Независим от allowedTabsOf —
 * можно разрешить вкладку, но не дать ни одного статуса внутри (вкладка будет
 * видна, но пустая), см. новую модель "вкладка / статус / подэтап" по отдельности.
 */
 ...
 * null — ограничений нет (видны все подэтапы, как и раньше). Независимый переключатель —
 * больше НЕ требует, чтобы родительский статус тоже был разрешён (см. allowedStatusesOf).
 */
