...
def call_llm(messages):
    """Вызывает LLM через HuggingFace Inference API."""
    payload = {
    ...
    resp = requests.post(HF_URL, json=payload, headers=headers, timeout=25)
    if resp.status_code != 200:
        raise Exception(f'HF error: {resp.status_code} {resp.text[:300]}')
