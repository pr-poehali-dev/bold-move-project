...
def handler(event, context):
    ...
    if is_plan_mode:
        ...
        plan_system = (
            ...
            "ПРАВИЛО 2 — ВЫБОР ПРОФИЛЕЙ:\n"
            "• «еврокраб», «еврокрап», «евро краб», «euro crab» → всегда «EuroKRAB стеновой» (НЕ потолочный)\n"
            "• «классика», «классик», «флекси классика» → «Теневой классик (Flexy KLASSIKA 140)»\n"
            "• «парящий», «парение» → профиль из категории «Парящий профиль»\n"
            "• «стеновой», «алюминиевый профиль», «обычный профиль» → «Стеновой алюминиевый»\n"
            "• «теневой» без уточнения → «Теневой классик (Flexy KLASSIKA 140)»\n"
...
- ЗАМЕНА позиции: если клиент просит заменить одну позицию на другую (например "замени X на Y") — новая позиция Y должна иметь то же qty что и удаляемая X. Смотри qty удаляемой позиции в текущей смете выше и используй его.
...
mounting_name — выбери из списка по типу товара:
- Карниз / ниша / KARNIZ / ПК-14 / ПК-12 → "Монтаж ниши"
- Стандартный/алюминиевый профиль → "Монтаж профиля стандарт"
- Теневой профиль → "Монтаж теневого профиля"
- Парящий профиль → "Монтаж парящего профиля"
- LED лента → "Монтаж ленты"
...
mounting_unit: пог.м или шт"""
            ...
            # ── Универсальная синхронизация монтажа через mounting_id из БД ──
            # Для каждой добавленной/обновлённой позиции проверяем:
            # есть ли у неё mounting_id → монтажная позиция должна иметь то же qty
            _rules_for_sync = get_price_rules()
            _id_to_rule = {r['id']: r for r in _rules_for_sync}
            # Строим маппинг: имя позиции (lower) → правило из БД
            ...
            for item in new_items:
                ...
                if existing_mount:
                    if existing_mount['qty'] != mount_qty:
                        print(f"[edit] sync mounting '{mount_name}': {existing_mount['qty']} → {mount_qty}")
                    ...
                    print(f"[edit] auto-added mounting '{mount_name}' qty={mount_qty}")
