...
from shared import (CORS, MAX_BODY_BYTES, OWNER_EMAIL, SCHEMA, SUPPORTED_EVENT_VERSIONS,
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if resource in ("status", "checksums", "events", "gaps") and method == "GET":
        ...
        if resource == "import":
            if not _admin_authorized(conn, event):
                return fail(401, "unauthorized")
            return _bulk_import(conn, json.loads(_raw_body(event) or "{}"))
...
def _purge_sandbox(conn, body):
    ...
    with conn.cursor() as c:
        c.execute(
            f"""DELETE FROM {SCHEMA}.live_chats WHERE id IN (
                    SELECT internal_id FROM {SCHEMA}.leakad_entities
                    WHERE account_id=%s AND entity_type='lead' AND internal_id IS NOT NULL)""",
            (account_id,))
        removed["live_chats"] = c.rowcount
        for table in ("leakad_comments", "leakad_tasks", "leakad_files",
                      "leakad_payments", "leakad_entities", "leakad_snapshots"):
            c.execute(f"DELETE FROM {SCHEMA}.{table} WHERE account_id=%s", (account_id,))
            removed[table] = c.rowcount
        c.execute(f"DELETE FROM {SCHEMA}.leakad_events WHERE account_id=%s", (account_id,))
        removed["leakad_events"] = c.rowcount
    conn.commit()
    return {"ok": True, "account_id": account_id, "removed": removed}
